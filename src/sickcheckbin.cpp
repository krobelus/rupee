...
void parseBinaryDrat() {
    if(Sickcheck.state >= 0) {
        ...
        std::cout << "Parsing binary DRAT proof" << std::endl;
        clause = (int*) malloc((Sickcheck.variables + 1) * sizeof(int));
        inputfile = std::fopen(Sickcheck.drat.c_str(), "r");
        if(inputfile == NULL) {
        ...
        while(true) {
            if(!inclause) {
                if((readchar = getc(inputfile)) == 100) {
                    deletion = true;
                } else if(readchar == 97) {
                    deletion = false;
                ...
                inclause = true;
                 ...
                 if((readchar = getc(inputfile)) == 0) {
                    *nextclause++ = 0;
                    ...
                    literal = readchar & 127;
                    if(readchar > 127) {
                        shift = 7;
                        do {
                            if((readchar = getc(inputfile)) == 0 || readchar == EOF) {
                                badParsing("DRAT");
                                fclose(inputfile);
                                return;
                            } else {
                                literal |= (readchar & 127) << shift;
                                shift += 7;
                            }
                        } while(readchar > 127);
                    }
