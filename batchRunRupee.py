...
tool = sys.argv[1]
if tool == "rupee-sd":
	instancelist = map(lambda x : x[:-4], os.listdir("test/drat"))
	...
	instancelist = map(lambda x : x[:-4], os.listdir("test/brat"))
	for x in instancelist:
	...
	instancelist = map(lambda x : x[:-4], os.listdir("test/lrat"))
	for x in instancelist:
		print "lratcheck on " + x ":"
		out = os.popen("bin/lratcheck test/cnf/" + x + ".cnf test/lrat/" + x + ".lrat")
		print out.read()
elif tool ==  "sickcheck":
	instancelist = map(lambda x : x[:-4], os.listdir("test/sick"))
	for x in instancelist:
