...
induction Tree; auto; simpl; intros.
elim (H t); right; left; auto.
Qed.

Lemma BT_singleton_char : forall Tree t, BT_Wf Tree -> (BT_In t Tree) ->
     (forall (t':T), (BT_In t' Tree) -> t=t') -> Tree = (node t nought nought).
intro Tree; case Tree; clear Tree.
intros.
inversion H0.
intros t TL TR t' Hwf H H0.
elim Hwf; clear Hwf; intros HT1 Hwf.
elim Hwf; clear Hwf; intros HT2 Hwf.
elim Hwf; clear Hwf; intros HTr1 HTr2.

rewrite (H0 t).
replace TL with nought.
replace TR with nought; auto.
symmetry; apply BT_nought_char.
intros; intro.
generalize (HTr2 _ H1); intro.
rewrite <- (H0 t) in H2.
rewrite <- (H0 t0) in H2.
rewrite T_compare_eq in H2; inversion H2.
right; right; auto.
right; left; auto.
symmetry; apply BT_nought_char.
intros; intro.
generalize (HTr1 _ H1); intro.
rewrite <- (H0 t) in H2.
rewrite <- (H0 t0) in H2.
rewrite T_compare_eq in H2; inversion H2.
left; auto.
...
set (Z := T_compare t' t); assert (Z = T_compare t' t); auto.
(* 1/3 *)
clearbody Z; revert H2; induction Z; intros; auto.
revert H H0; case Tree2; intros.
inversion_clear H0; auto.
inversion_clear H3; auto.
elim H1; rewrite (eq_T_compare _ _ (eq_sym H2)); auto.
inversion H0.
rename Tree1 into TL; rename b into TRL; rename b0 into TRR.
set (Z := BT_split (node t1 TRL TRR) t'); assert (Z = BT_split (node t1 TRL TRR) t'); auto.
clearbody Z; revert H3; induction Z; intros; auto.
inversion_clear H0.
left; auto.
inversion_clear H4.
elim H1; rewrite (eq_T_compare _ _ (eq_sym H2)); auto.
elim (BT_split_in _ _ _ _ _ (eq_sym H3) H0); intros.
right; left; auto.
right; right; auto.
(* 2/3 *)
...
induction Tree.
simpl; intros; intro; auto.

simpl; intro.
elim H; clear H; intros HT1 H.
elim H; clear H; intros HT2 H.
elim H; clear H; intros HTr1 HTr2.

set (Z := T_compare t t0); assert (Z = T_compare t t0); auto.
induction Z.
induction Tree2; intro.
generalize (HTr1 _ H0); intro.
rewrite H1 in H; inversion H.
set (Z := BT_split (node t1 Tree2_1 Tree2_2) t); assert (Z = BT_split (node t1 Tree2_1 Tree2_2) t); auto.
clearbody Z; induction Z; intros.
rewrite <- H1 in H0.
clear IHTree2_1 IHTree2_2.
inversion_clear H0.
generalize (HTr1 _ H2); intro.
rewrite H0 in H; inversion H.
inversion_clear H2.
set (Tree2 := (node t1 Tree2_1 Tree2_2)); fold Tree2 in H1, HTr2, HT2, IHTree2.
generalize (BT_split_in_l a t Tree2); intro.
rewrite HTr2 in H.
inversion H.
rewrite H0; apply H2 with b; auto.
unfold Tree2; discriminate.
set (Tree2 := (node t1 Tree2_1 Tree2_2)); fold Tree2 in H1, HTr2, HT2, IHTree2.
generalize (BT_split_in_r a t t Tree2); intro.
rewrite HTr2 in H.
inversion H.
...
apply IHTree1; auto.
inversion_clear H1.
rewrite H0 in H; rewrite T_compare_eq in H; inversion H.
rewrite HTr2 in H; auto; inversion H.
(* 3 *)
intro.
inversion_clear H0.
rewrite HTr1 in H; auto; inversion H.
inversion_clear H1.
rewrite H0 in H; rewrite T_compare_eq in H; inversion H.
apply IHTree2; auto.
Qed.
...
intros Tree Tree'; revert Tree; induction Tree'.
intros; inversion H0.
simpl; intros.
intro.
generalize (BT_remove_in t t0 _ _ (eq_refl _) H1); intro.
inversion_clear H0.
generalize (BT_diff_in _ _ _ _ (eq_refl _) H2).
apply IHTree'1; auto.
inversion_clear H3.
revert H1; rewrite H0; apply BT_remove_out.
eapply BT_diff_wf; auto.
eapply BT_diff_wf; auto.
auto.
revert H2; apply IHTree'2; auto.
eapply BT_diff_wf; auto.
auto.
Qed.
...
intros; intro.
revert H0.
apply BT_diff_out; auto.
Qed.

Lemma BT_diff_nought : forall Tree Tree', BT_Wf Tree ->
     (forall t:T, BT_In t Tree -> BT_In t Tree') -> BT_diff Tree Tree' = nought.
intros; apply BT_nought_char.
intros; intro.
generalize (BT_diff_in _ _ _ _ (eq_refl _) H1); intro.
revert H1; apply BT_diff_out; auto.
Qed.

Lemma BT_nought_diff : forall Tree Tree', BT_Wf Tree ->
  BT_diff Tree Tree' = nought -> forall t:T, BT_In t Tree -> BT_In t Tree'.
intros Tree Tree'; revert Tree; induction Tree'; simpl; intros.
rewrite H0 in H1; apply H1.
generalize (BT_diff_in_rev _ _ _ (eq_refl (BT_diff Tree Tree'1)) _ H1); intros.
clear H1; inversion_clear H2; auto.
generalize (BT_diff_in_rev _ _ _ (eq_refl (BT_diff (BT_diff Tree Tree'1) Tree'2)) _ H1); intros.
clear H1; inversion_clear H2; auto.
generalize (BT_remove_in_rev _ _ _ H0 _ H1); intros.
clear H1; inversion_clear H2; auto.
inversion H1.
Qed.
